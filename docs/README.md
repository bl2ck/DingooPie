...
修改窗口缩放、全屏、抗锯齿、滤镜、亮度、对比度、伽马、饱和度、最小化时、
竖屏模式、FPS 显示、音频、输入、CPU 时钟、运行速度、延迟比例、金手指总开关、
语言、调试控制台和性能日志会保存并立即生效。`DingooPie.ini` 写出顺序为
`recent`、`video`、`audio`、`input`、`runtime`、可选 `cheats`、`ui`、`debug`。
...
gamma, saturation, minimized behavior, portrait mode, FPS overlay, audio, input, CPU
clock, runtime speed, delay scale, the global cheat switch, debug console, and
performance log settings save and apply immediately. `DingooPie.ini` is written
as `recent`, `video`, `audio`, `input`, `runtime`, optional `cheats`, `ui`, and
`debug`. Changing the CPU backend saves the setting and relaunches the emulator
because the execution backend is selected at startup.
...
file. The menu shows saved slot times down to seconds. Saving and loading ask
for confirmation first. Loading validates the game and runtime-state layout. If the
