...
- `设置`：CPU 后端、CPU 时钟、运行速度、延迟比例、语言和恢复默认设置。
- `调试`：调试控制台、性能日志和打开调试日志。
- `帮助`：关于 丁果派 DingooPie。

修改窗口缩放、全屏、抗锯齿、滤镜、亮度、对比度、饱和度、最小化时、
竖屏模式、FPS 显示、音频、输入、CPU 时钟、运行速度、延迟比例、
...
清晰档使用线性采样和轻度清晰补偿。未知或非法 INI 值不做特殊兼容，
会按当前默认值兜底并在下次保存时写回当前格式。
...
`SDL2.dll`、`libcapstone.dll`、`libwinpthread-1.dll`、`README.md` 和
`manifest.sha256`。缺少必要运行时 DLL 会导致发布失败。
...
clock, runtime speed, delay scale, debug console, and profile log settings save
and apply immediately. Changing the CPU backend saves the setting and
...
Unknown or invalid INI values are not specially mapped; they fall back to
current defaults and are rewritten in the current format on the next save.
...
`libcapstone.dll`, `libwinpthread-1.dll`, `README.md`, and `manifest.sha256`.
