...
- Release binaries and required DLLs
- Release and package SHA256 manifests

Dependency archives and extracted dependency trees such as `downloads/`,
`third_party/`, `deps_extract/`, and `w64devkit/` are local build workspaces.
They can be regenerated by `scripts/bootstrap_windows.ps1` and are not treated
as project source.
