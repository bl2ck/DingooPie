...
{
#ifdef _WIN32
    return _stricmp(gamePathNormalize(a.c_str()).c_str(), gamePathNormalize(b.c_str()).c_str()) == 0;
#else
...
static bool writeOrderedSettingsFile(const EmulatorSettings& settings, const std::string& path)
{
    ...
    appendIniValue(&text, L"screen_fill", emulatorScreenFillName(settings.screenFill));
    ...
    appendIniValue(&text, L"digital_noise_reduction",
    ...
    appendIniValue(&text, L"system_ime_disabled", settings.systemImeDisabled);
...
static bool settingsPathExists(const std::string& path)
{
    DWORD attributes = GetFileAttributesW(platformUtf8ToWide(path).c_str());
...
static void recoverSettingsTransaction(const std::string& path)
{
    ...
    const std::wstring pathW = platformUtf8ToWide(path);
    const std::wstring temporaryPathW = platformUtf8ToWide(temporaryPath);
    const std::wstring backupPathW = platformUtf8ToWide(backupPath);
    if (!settingsPathExists(path))
    {
        if (settingsPathExists(temporaryPath))
        {
            MoveFileExW(temporaryPathW.c_str(), pathW.c_str(),
                MOVEFILE_REPLACE_EXISTING | MOVEFILE_WRITE_THROUGH);
        }
        if (!settingsPathExists(path) && settingsPathExists(backupPath))
        {
            CopyFileW(backupPathW.c_str(), pathW.c_str(), FALSE);
        }
    }
    else if (settingsPathExists(temporaryPath))
    {
        DeleteFileW(temporaryPathW.c_str());
    }
}

static bool writeSettingsTransaction(
    const EmulatorSettings& settings, const std::string& path)
{
    const std::string temporaryPath = path + ".tmp";
    const std::string backupPath = path + ".backup";
    const std::wstring pathW = platformUtf8ToWide(path);
    ...
    const std::wstring backupPathW = platformUtf8ToWide(backupPath);
    DeleteFileW(temporaryPathW.c_str());
    if (!writeOrderedSettingsFile(settings, temporaryPath) ||
        !settingsPathExists(temporaryPath))
    {
        DeleteFileW(temporaryPathW.c_str());
        return false;
    }
    if (settingsPathExists(path) &&
        !CopyFileW(pathW.c_str(), backupPathW.c_str(), FALSE))
    {
        DeleteFileW(temporaryPathW.c_str());
        return false;
    }
    if (!MoveFileExW(temporaryPathW.c_str(), pathW.c_str(),
            MOVEFILE_REPLACE_EXISTING | MOVEFILE_WRITE_THROUGH))
    {
        DeleteFileW(temporaryPathW.c_str());
        if (!settingsPathExists(path) && settingsPathExists(backupPath))
...
EmulatorSettings emulatorLoadSettings(void)
{
    ...
    settings.audioBufferLatency = parseAudioBufferLatencyMode(
        readIniString("audio", "buffer_latency",
            emulatorAudioBufferLatencyName(defaults.audioBufferLatency), path),
    ...
    settings.systemImeDisabled = parseBoolText(readIniString(
        "input", "system_ime_disabled", defaults.systemImeDisabled ? "1" : "0", path).c_str(),
        defaults.systemImeDisabled);
    ...
    settings.portraitMode = settings.screenOrientationMode == SCREEN_ORIENTATION_PORTRAIT;
    return settings;
}
...
bool emulatorSaveSettings(const EmulatorSettings& settings)
{
    std::string path = emulatorSettingsPath();
#ifdef _WIN32
    bool ok = writeSettingsTransaction(settings, path);
#else
    bool ok = true;
    ...
    ok = writeIniString("input", "system_ime_disabled", settings.systemImeDisabled ? "1" : "0", path) && ok;
...
void emulatorTraceSettings(const char* reason, const EmulatorSettings& settings)
{
    ...
    printf("settings-trace:%s video.scale=%d video.fullscreen=%u video.anti_aliasing=%s video.effect=%s video.brightness=%d video.contrast=%d video.gamma=%d video.saturation=%d video.minimized_behavior=%s video.screen_orientation=%s video.screen_fill=%s video.show_fps=%u\n",
        label,
