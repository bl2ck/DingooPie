    ...
    uint32_t* unresolvedOut)
{
    ...
    for (size_t offset = 0; offset + sizeof(uint32_t) <= region.data.size(); offset += sizeof(uint32_t))
    {
        ...
        {
            uint32_t current = loadStateLe32(target.data + targetOffset + offset);
...
bool appRuntimeRestoreState(const AppRuntimeState& state, std::string* error)
{
    ...
    for (size_t i = 0; i < state.regions.size(); ++i)
    {
        ...
        for (size_t j = 0; j < runtimeRegions.size(); ++j)
        {
            ...
            if (!usedRuntimeRegions[j] && runtimeRegion.start == region.start &&
                runtimeRegion.size == region.size)
            {
                restoreTargets[i] = j;
                usedRuntimeRegions[j] = true;
                break;
            }
        }
        ...
        uint32_t restoredMarkers = collectRestoredIrJitMarkerReplacements(
            runtimeRegions[restoreTargets[i]], region, &markerReplacements[i],
            &unresolvedMarkers);
        if (restoredMarkers && getenv("DINGOO_PIE_IRJIT_TRACE"))
        {
            printf("save-state: prepared region 0x%08x replacement for %u/%u IRJIT marker(s), unresolved=%u\n",
                region.start,
                (unsigned)markerReplacements[i].size(),
                restoredMarkers,
                unresolvedMarkers);
        }
    }
    ...
    for (size_t i = 0; i < state.regions.size(); ++i)
    {
        ...
        {
            const RestoredIrJitMarkerReplacement& replacement = markerReplacements[i][j];
...
bool appRuntimeGetInfo(AppRuntimeInfo* out)
{
    ...
    *out = AppRuntimeInfo();
    ...
    out->fileName = gameFileNameFromPath(g_appLoadPath);
