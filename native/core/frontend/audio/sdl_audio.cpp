...
static int32_t g_noiseReductionPreviousInput[kAudioEffectStateChannels] = {};
static int32_t g_noiseReductionPreviousOutput[kAudioEffectStateChannels] = {};
static bool g_noiseReductionStateValid[kAudioEffectStateChannels] = {};
...
static void applyQueueRecoveryFadeInLocked(char* buffer, int count, uint32_t queuedBytes)
{
    if (!buffer || count <= 0 || queuedBytes != 0 ||
        g_audioSpec.format != AUDIO_S16SYS || g_audioSpec.channels == 0)
    {
        return;
    }
    int16_t* samples = (int16_t*)buffer;
    const int sampleCount = count / (int)sizeof(int16_t);
    const int frameCount = sampleCount / g_audioSpec.channels;
    const int fadeFrames = frameCount < 128 ? frameCount : 128;
    for (int frame = 0; frame < fadeFrames; ++frame)
    {
        for (int channel = 0; channel < g_audioSpec.channels; ++channel)
        {
            int index = frame * g_audioSpec.channels + channel;
            samples[index] = (int16_t)((int32_t)samples[index] * (frame + 1) / fadeFrames);
        }
    }
}
...
static uint32_t audioBufferLatencyMillisecondsLocked(void)
{
    ...
    return (uint32_t)emulatorAudioBufferLatencyMilliseconds(g_audioBufferLatencyMode);
}
...
static void resetAudioEffectStateLocked(void)
{
    ...
    memset(g_noiseReductionPreviousInput, 0, sizeof(g_noiseReductionPreviousInput));
    memset(g_noiseReductionPreviousOutput, 0, sizeof(g_noiseReductionPreviousOutput));
    memset(g_noiseReductionStateValid, 0, sizeof(g_noiseReductionStateValid));
}
...
static int16_t applyNoiseReductionSampleLocked(int16_t sample, int channel)
{
    ...
    const int stateChannel = channel % kAudioEffectStateChannels;
    if (!g_noiseReductionStateValid[stateChannel])
    {
        g_noiseReductionPreviousInput[stateChannel] = sample;
        g_noiseReductionPreviousOutput[stateChannel] = 0;
        g_noiseReductionStateValid[stateChannel] = true;
        return 0;
    }

    int32_t output = sample - g_noiseReductionPreviousInput[stateChannel] +
        (g_noiseReductionPreviousOutput[stateChannel] * 32700) / 32768;
    g_noiseReductionPreviousInput[stateChannel] = sample;
    g_noiseReductionPreviousOutput[stateChannel] = output;
...
static void applyNoiseReductionInPlaceLocked(char* buffer, int count)
{
    if (!buffer || count <= 0 || g_digitalNoiseReduction == DIGITAL_NOISE_REDUCTION_LOW)
    {
        return;
    }
    ...
    if (g_audioSpec.format == AUDIO_S16LSB)
    {
        ...
        {
            samples[index] = applyNoiseReductionSampleLocked(samples[index], index % channels);
        }
    }
    else if (g_audioSpec.format == AUDIO_U8)
    {
        ...
        {
            int16_t centered = (int16_t)(((int)samples[index] - 128) << 8);
            int16_t processed = applyNoiseReductionSampleLocked(centered, index % channels);
            samples[index] = (uint8_t)clampIntLocal(128 + ((int)processed >> 8), 0, 255);
        }
    }
}
...
uint32_t MixerWriteBuff(char* buffer, int count)
{
    ...
    applyNoiseReductionInPlaceLocked(buffer, count);
    ...
    applyQueueRecoveryFadeInLocked(buffer, count, queuedBytes);
...
void MixerSetBufferLatencyMode(AudioBufferLatencyMode mode)
{
    lockAudio();
    ...
    SDL_Log("Audio buffer latency set to %s (%u ms)",
        emulatorAudioBufferLatencyName(g_audioBufferLatencyMode),
        (unsigned int)audioBufferLatencyMillisecondsLocked());
    unlockAudio();
}

void MixerSetNoiseReduction(DigitalNoiseReductionLevel level)
{
    lockAudio();
    if (level < DIGITAL_NOISE_REDUCTION_HIGH ||
        level >= DIGITAL_NOISE_REDUCTION_LEVEL_COUNT)
    {
        level = DIGITAL_NOISE_REDUCTION_HIGH;
    }
