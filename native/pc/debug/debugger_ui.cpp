...
static void refreshMemory(uint32_t address, uint32_t bytes)
{
    ...
    for (uint32_t row = 0; row < rows; ++row)
    {
        ...
        bool ok = gameRuntimeReadMemory(rowAddress, buffer, sizeof(buffer));
...
static void refreshDebugger(void)
{
    ...
    refreshRegisters(snapshot, arm32);
    ...
    {
        memAddress = snapshot.gpr[arm32 ? 13 : 29];
    ...
    refreshDebugEntryLists();
    setDebuggerStatus(arm32 ?
        (debuggerChinese() ?
            L"CC ARM32 \u5df2\u5237\u65b0\uff1b\u5f53\u524d\u652f\u6301\u5bc4\u5b58\u5668\u548c\u5185\u5b58\u67e5\u770b\uff0c\u65ad\u70b9\u4ec5\u9002\u7528\u4e8e APP\u3002" :
            L"CC ARM32 refreshed. Registers and memory are available; breakpoints are APP-only.") :
        (debuggerChinese() ?
...
{
    if (!gameRuntimeSupportsBreakpoints())
    {
        setDebuggerStatus(debuggerChinese() ?
            L"CC \u6682\u4e0d\u652f\u6301 PC \u547d\u4e2d\uff1b\u8be5\u529f\u80fd\u4ec5\u9002\u7528\u4e8e APP\u3002" :
            L"CC does not support PC hits; this feature is APP-only.");
        return;
    }
    uint32_t address = 0;
...
static void addWriteHitFromInput(void)
{
    ...
    {
        setDebuggerStatus(debuggerChinese() ?
            L"CC \u6682\u4e0d\u652f\u6301\u5199\u5165\u547d\u4e2d\uff1b\u8be5\u529f\u80fd\u4ec5\u9002\u7528\u4e8e APP\u3002" :
            L"CC does not support write hits; this feature is APP-only.");
        return;
    }
    uint32_t address = 0;
    uint32_t size = 0;
