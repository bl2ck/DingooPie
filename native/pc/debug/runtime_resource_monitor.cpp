...
static RuntimeResourceMonitorEntry* ensureGuestMonitorEntryLocked(
    const GuestResourceEntry* entry)
{
    ...
    {
        RuntimeResourceMonitorEntry& item = g_resourceMonitorSnapshot.entries[i];
        if (!item.appPackageSeen && !item.externalFileSeen &&
            item.name == entry->name && item.offset == entry->offset &&
            item.size == entry->size)
        {
            return &item;
        }
    ...
    item.size = entry->size;
    item.xorKey = entry->xorKey;
    item.cached = entry->decoded_data != NULL || entry->xorKey == 0;
    item.firstRevision = g_resourceMonitorSnapshot.revision + 1;
    g_resourceMonitorSnapshot.entries.push_back(item);
    return &g_resourceMonitorSnapshot.entries.back();
}
...
void runtimeResourceMonitorSetGuestResources(GuestPackage* package)
{
    ...
    if (package)
    {
        ...
        {
            ensureGuestMonitorEntryLocked(&package->resource_data[i]);
        }
    }
    g_resourceMonitorSnapshot.revision++;
    ...
    const char* requestName, const GuestResourceEntry* entry, bool cached)
{
    ...
    pthread_mutex_lock(&g_resourceMonitorMutex);
    RuntimeResourceMonitorEntry* item = ensureGuestMonitorEntryLocked(entry);
    if (item)
    {
        ...
        item->activeHandles++;
        item->openCount++;
        uint64_t revision = recordMonitorEntryActionLocked(
            item, RUNTIME_RESOURCE_MONITOR_SOURCE_DL_RES, "open");
        if (firstOpen)
        {
    ...
    const void* data, uint32_t bytesRead, uint32_t positionAfter)
{
    ...
    if (item)
    {
        updateMonitorEntryLoadLocked(
            item, guestAddress, data, bytesRead, positionAfter);
        item->lastReadRevision = recordMonitorEntryActionLocked(
            item, RUNTIME_RESOURCE_MONITOR_SOURCE_DL_RES, "load");
    }
    pthread_mutex_unlock(&g_resourceMonitorMutex);
}
...
void runtimeResourceMonitorRecordGuestClose(const GuestResourceEntry* entry)
{
    ...
    if (item)
    {
        ...
        item->lastCloseRevision = recordMonitorEntryActionLocked(
            item, RUNTIME_RESOURCE_MONITOR_SOURCE_DL_RES, "close");
    }
    pthread_mutex_unlock(&g_resourceMonitorMutex);
}
