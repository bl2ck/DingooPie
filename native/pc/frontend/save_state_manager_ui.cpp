...
static std::wstring saveStateManagerSlotLabel(int slot, bool exists, uint64_t modifiedTime)
{
    ...
    else
    {
        ...
        {
            swprintf(text, sizeof(text) / sizeof(text[0]),
                L"Slot %d%ls", slot, exists ? L"" : L" (Empty)");
        }
    }
    return std::wstring(text);
}
...
static void deleteSelectedSaveStateSlot(void)
{
    ...
    std::wstring message = saveStateManagerChinese() ?
        L"\u6240\u9009\u5b58\u6863\u548c\u622a\u56fe\u5c06\u88ab\u6c38\u4e45\u5220\u9664\u3002\u662f\u5426\u7ee7\u7eed\uff1f" :
        L"The selected state and screenshot will be permanently deleted. Continue?";
    message += L"\n";
    ...
    bool ok = saveStateDeleteSlot(g_saveStateManagerAppPath,
        saveStateFormatForPath(g_saveStateManagerAppPath), slot);
    refreshSaveStateManagerList();
    notifySaveStateManagerChanged();
    setSaveStateManagerStatus(ok ?
        (saveStateManagerChinese() ? L"\u5b58\u6863\u5df2\u5220\u9664\u3002" : L"State deleted.") :
        (saveStateManagerChinese() ? L"\u65e0\u6cd5\u5220\u9664\u5b58\u6863\u3002" : L"Could not delete state."));
}
