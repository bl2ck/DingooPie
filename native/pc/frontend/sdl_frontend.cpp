...
struct ControllerAutotestState
{
    int virtualControllerIndex = -1;
    bool buttonMappingStarted = false;
    bool buttonMappingCaptureReleased = false;
    bool mappedButtonPressed = false;
    bool mappedButtonReleased = false;
    bool axisMappingStarted = false;
    bool axisMappingCaptureReleased = false;
    bool mappedAxisPressed = false;
    bool mappedAxisReleased = false;
    bool calibrationStarted = false;
    bool summaryLogged = false;
};
static ControllerAutotestState g_controllerAutotest;
...
static bool portraitModeEnabled(void)
{
    ...
    if (g_frontendSettings->screenOrientationMode == SCREEN_ORIENTATION_PORTRAIT)
    {
        return true;
    }
    if (g_frontendSettings->screenOrientationMode != SCREEN_ORIENTATION_AUTO || !g_renderer)
    {
        return false;
    }
    int width = 0;
...
static bool controllerAutotestEnabled(void)
{
    static int enabled = -1;
    if (enabled < 0)
    {
        const char* value = getenv("DINGOO_PIE_AUTOTEST_CONTROLLER");
        enabled = value && value[0] && value[0] != '0' ? 1 : 0;
    }
    return enabled != 0;
}

static void attachAutotestVirtualController(void)
{
    if (!controllerAutotestEnabled() || g_controllerAutotest.virtualControllerIndex >= 0)
    {
        return;
    }

    g_controllerAutotest = ControllerAutotestState();
    g_controllerAutotest.virtualControllerIndex = SDL_JoystickAttachVirtual(
        SDL_JOYSTICK_TYPE_GAMECONTROLLER,
        SDL_CONTROLLER_AXIS_MAX,
        SDL_CONTROLLER_BUTTON_MAX,
        0);
    if (g_controllerAutotest.virtualControllerIndex < 0)
    {
        printf("frontend-autotest: failed to attach virtual controller: %s\n", SDL_GetError());
        return;
    }
    printf("frontend-autotest: virtual controller attached index=%d\n",
        g_controllerAutotest.virtualControllerIndex);
}

static void detachAutotestVirtualController(void)
{
    if (g_controllerAutotest.virtualControllerIndex >= 0 &&
        SDL_JoystickDetachVirtual(g_controllerAutotest.virtualControllerIndex) < 0)
    {
        printf("frontend-autotest: failed to detach virtual controller: %s\n", SDL_GetError());
    }
    g_controllerAutotest = ControllerAutotestState();
}
...
static void applyPixelGridToDisplayPixels(uint16_t* pixels, int width, int height, int sourceWidth, int sourceHeight)
{
    ...
    for (int y = 0; y < height; ++y)
    {
        ...
        for (int x = 0; x < width; ++x)
        {
            ...
            pixels[index] = frontendBlendRgb565WithBlack(pixels[index], alpha);
        }
    }
}
...
static const size_t kControllerMappingSourceCount =
    sizeof(kControllerMappingSources) / sizeof(kControllerMappingSources[0]);
static_assert(kControllerMappingSourceCount ==
    SDL_CONTROLLER_BUTTON_MAX + SDL_CONTROLLER_AXIS_MAX * 2 - 2,
    "Controller mapping sources must cover every SDL controller button and axis direction");
...
static void applyControllerCalibrationSettings(const std::string& calibration)
{
    if (g_controllerCalibrationInitialized &&
        calibration == g_appliedControllerCalibration)
    {
        return;
    }
...
static Sint16 calibratedControllerAxisValue(int axis, Sint16 rawValue)
{
    ...
    const ControllerAxisCalibration& calibration =
        g_controllerAxisCalibration[axis];
...
bool frontendControllerCalibrationActive(void)
{
    return g_controllerCalibrationStage != CONTROLLER_CALIBRATION_IDLE;
}

void frontendCancelControllerCalibration(void)
{
    bool wasActive = frontendControllerCalibrationActive();
    ...
    inputMappingUiSetStatus(frontendUiLanguage() == UI_LANGUAGE_CHINESE ?
        L"\u624b\u67c4\u6447\u6746\u6821\u51c6\u5df2\u53d6\u6d88\u3002" : L"Joystick calibration cancelled.");
    inputMappingUiRefresh();
#endif
    printf("frontend: controller calibration cancelled\n");
}

void frontendResetControllerCalibration(void)
{
    frontendCancelControllerCalibration();
    setDefaultControllerCalibration();
    g_controllerCalibrationInitialized = true;
    ...
    inputMappingUiSetStatus(frontendUiLanguage() == UI_LANGUAGE_CHINESE ?
        L"\u624b\u67c4\u6447\u6746\u6821\u51c6\u5df2\u6062\u590d\u9ed8\u8ba4\u3002" :
        L"Joystick calibration restored to defaults.");
    inputMappingUiRefresh();
#endif
    printf("frontend: controller calibration reset to defaults\n");
}

void frontendBeginControllerCalibration(void)
{
    ...
    if (!g_gameController)
    {
        ...
        inputMappingUiSetStatus(frontendUiLanguage() == UI_LANGUAGE_CHINESE ?
            L"\u672a\u68c0\u6d4b\u5230\u517c\u5bb9\u624b\u67c4\u3002" :
            L"No compatible controller detected.");
        MessageBoxW(g_nativeWindow,
            frontendUiLanguage() == UI_LANGUAGE_CHINESE ?
                L"\u672a\u68c0\u6d4b\u5230\u517c\u5bb9\u624b\u67c4\u3002" :
                L"No compatible controller detected.",
            frontendUiLanguage() == UI_LANGUAGE_CHINESE ?
                L"\u624b\u67c4\u6447\u6746\u6821\u51c6" : L"Joystick Calibration",
            MB_OK | MB_ICONINFORMATION);
#endif
        printf("frontend: controller calibration unavailable because no SDL GameController is connected\n");
        return;
    }
    ...
    inputMappingUiSetStatus(frontendUiLanguage() == UI_LANGUAGE_CHINESE ?
        L"\u8bf7\u677e\u5f00\u6240\u6709\u6447\u6746\uff0c\u4fdd\u6301\u9759\u6b62\u2026" :
        L"Release all sticks and keep them centered\u2026");
    inputMappingUiRefresh();
#endif
    printf("frontend: controller calibration started stage=center\n");
}
...
static void finishControllerCalibration(void)
{
    ...
    inputMappingUiSetStatus(frontendUiLanguage() == UI_LANGUAGE_CHINESE ?
        L"\u6821\u51c6\u5df2\u5b8c\u6210\u5e76\u4fdd\u5b58\u3002" :
        L"Calibration completed and saved.");
    inputMappingUiRefresh();
    if (!controllerAutotestEnabled())
    {
        MessageBoxW(g_nativeWindow,
            frontendUiLanguage() == UI_LANGUAGE_CHINESE ?
                L"\u6821\u51c6\u5df2\u5b8c\u6210\u5e76\u4fdd\u5b58\u3002" :
...
#endif
    printf("frontend: controller calibration saved spec='%s'\n",
        g_frontendSettings && !g_frontendSettings->controllerCalibration.empty() ?
            g_frontendSettings->controllerCalibration.c_str() : "(default)");
}

static void updateControllerCalibration(void)
{
    if (!frontendControllerCalibrationActive() || !g_gameController)
    {
        return;
    }
    if (controllerAutotestEnabled())
    ...
    if (g_controllerCalibrationStage == CONTROLLER_CALIBRATION_CENTER)
    {
        ...
        if (now - g_controllerCalibrationStageStartTicks >= kControllerCalibrationCenterDurationMs)
        {
            for (int axis = 0; axis < kControllerCalibrationAxisCount; ++axis)
            {
            ...
            inputMappingUiSetStatus(frontendUiLanguage() == UI_LANGUAGE_CHINESE ?
                L"\u8bf7\u5c06\u6240\u6709\u6447\u6746\u6cbf\u6700\u5927\u8303\u56f4\u7f13\u6162\u8f6c\u52a8\u2026" :
                L"Move every stick slowly through its full range\u2026");
            inputMappingUiRefresh();
#endif
            printf("frontend: controller calibration stage=range\n");
        }
        return;
    }
...
static void runAutotestControllerActions(uint64_t elapsedMs)
{
    if (!controllerAutotestEnabled() || !g_gameController)
    {
        return;
    }
    ...
    if (!g_controllerAutotest.buttonMappingStarted && elapsedMs >= 100)
    {
        frontendBeginControllerMapping(CONTROL_BUTTON_A);
        SDL_JoystickSetVirtualButton(joystick, SDL_CONTROLLER_BUTTON_B, SDL_PRESSED);
        g_controllerAutotest.buttonMappingStarted = true;
    }
    if (g_controllerAutotest.buttonMappingStarted &&
        !g_controllerAutotest.buttonMappingCaptureReleased && elapsedMs >= 300)
    {
        SDL_JoystickSetVirtualButton(joystick, SDL_CONTROLLER_BUTTON_B, SDL_RELEASED);
        g_controllerAutotest.buttonMappingCaptureReleased = true;
    }
    if (g_controllerAutotest.buttonMappingCaptureReleased &&
        !g_controllerAutotest.mappedButtonPressed && elapsedMs >= 400)
    {
        SDL_JoystickSetVirtualButton(joystick, SDL_CONTROLLER_BUTTON_B, SDL_PRESSED);
        g_controllerAutotest.mappedButtonPressed = true;
    }
    if (g_controllerAutotest.mappedButtonPressed &&
        !g_controllerAutotest.mappedButtonReleased && elapsedMs >= 500)
    {
        SDL_JoystickSetVirtualButton(joystick, SDL_CONTROLLER_BUTTON_B, SDL_RELEASED);
        g_controllerAutotest.mappedButtonReleased = true;
    }

    if (!g_controllerAutotest.axisMappingStarted && elapsedMs >= 650)
    {
        frontendBeginControllerMapping(CONTROL_BUTTON_X);
        SDL_JoystickSetVirtualAxis(joystick, SDL_CONTROLLER_AXIS_RIGHTX, 30000);
        g_controllerAutotest.axisMappingStarted = true;
    }
    if (g_controllerAutotest.axisMappingStarted &&
        !g_controllerAutotest.axisMappingCaptureReleased && elapsedMs >= 850)
    {
        SDL_JoystickSetVirtualAxis(joystick, SDL_CONTROLLER_AXIS_RIGHTX, 0);
        g_controllerAutotest.axisMappingCaptureReleased = true;
    }
    if (g_controllerAutotest.axisMappingCaptureReleased &&
        !g_controllerAutotest.mappedAxisPressed && elapsedMs >= 1000)
    {
        SDL_JoystickSetVirtualAxis(joystick, SDL_CONTROLLER_AXIS_RIGHTX, 30000);
        g_controllerAutotest.mappedAxisPressed = true;
    }
    if (g_controllerAutotest.mappedAxisPressed &&
        !g_controllerAutotest.mappedAxisReleased && elapsedMs >= 1150)
    {
        SDL_JoystickSetVirtualAxis(joystick, SDL_CONTROLLER_AXIS_RIGHTX, 0);
        g_controllerAutotest.mappedAxisReleased = true;
    }

    static const Sint16 kCenters[kControllerCalibrationAxisCount] = { 800, -600, 1200, -1000 };
    if (!g_controllerAutotest.calibrationStarted && elapsedMs >= 1300)
    {
        for (int axis = 0; axis < kControllerCalibrationAxisCount; ++axis)
        {
            SDL_JoystickSetVirtualAxis(joystick, axis, kCenters[axis]);
        }
        frontendBeginControllerCalibration();
        g_controllerAutotest.calibrationStarted = true;
    }

    if (frontendControllerCalibrationActive())
    {
        if (g_controllerCalibrationStage == CONTROLLER_CALIBRATION_CENTER)
        {
            for (int axis = 0; axis < kControllerCalibrationAxisCount; ++axis)
        ...
        {
            Sint16 rangeValue = ((elapsedMs / 180) & 1) ? 30000 : -30000;
            for (int axis = 0; axis < kControllerCalibrationAxisCount; ++axis)
            {
                SDL_JoystickSetVirtualAxis(joystick, axis,
                    axis & 1 ? (Sint16)-rangeValue : rangeValue);
            }
        }
    }
    else if (g_controllerAutotest.calibrationStarted && !g_controllerAutotest.summaryLogged)
    {
        printf("frontend-autotest: controller mapping='%s' calibration='%s'\n",
            g_frontendSettings && !g_frontendSettings->controllerMapping.empty() ?
                g_frontendSettings->controllerMapping.c_str() : "(default)",
            g_frontendSettings && !g_frontendSettings->controllerCalibration.empty() ?
                g_frontendSettings->controllerCalibration.c_str() : "(default)");
        g_controllerAutotest.summaryLogged = true;
    }
}
...
{
    for (size_t i = 0; i < kControllerMappingSourceCount; ++i)
    {
...
static void showControllerMappingPrompt(uint32_t targetMask)
{
    ...
    inputMappingUiSetStatus(body.c_str());
    inputMappingUiRefresh();
}
...
void frontendOpenInputMappingWindow(void)
{
    ...
    callbacks.cancelControllerMapping = cancelControllerMapping;
    callbacks.resetControllerMapping = frontendResetControllerMapping;
    callbacks.beginControllerCalibration = frontendBeginControllerCalibration;
    callbacks.resetControllerCalibration = frontendResetControllerCalibration;
    callbacks.cancelControllerCalibration = frontendCancelControllerCalibration;
    callbacks.controllerCalibrationActive = frontendControllerCalibrationActive;
...
#ifdef _WIN32
    inputMappingUiSetStatus(frontendUiLanguage() == UI_LANGUAGE_CHINESE ?
        L"\u624b\u67c4\u6620\u5c04\u5df2\u53d6\u6d88\u3002" : L"Controller mapping cancelled.");
    inputMappingUiRefresh();
#endif
}
...
static void handleGameControllerDeviceRemoved(SDL_JoystickID instanceId)
{
    ...
    if (frontendControllerCalibrationActive())
    {
        ...
        inputMappingUiSetStatus(frontendUiLanguage() == UI_LANGUAGE_CHINESE ?
            L"\u624b\u67c4\u5df2\u65ad\u5f00\uff0c\u6447\u6746\u6821\u51c6\u5df2\u53d6\u6d88\u3002" :
            L"Controller disconnected; stick calibration cancelled.");
#endif
    }
    closeGameController();
    openFirstGameController();
...
static void handleGameControllerAxisEvent(const SDL_ControllerAxisEvent& axis)
{
    ...
    Sint16 calibratedValue = calibratedControllerAxisValue(axis.axis, axis.value);
    g_gameControllerAxes[axis.axis] = calibratedValue;
    applyGameControllerControlMasks(g_gameControllerButtonControls, gameControllerAxisControlMask());

    if (inputTraceEnabled())
    {
        printf("frontend: controller axis=%u raw=%d calibrated=%d mask=0x%08X\n",
...
static int pcFpsOverlayScale(void)
{
    int width = 0;
    int height = 0;
    if (g_renderer)
    ...
    int baseWidth = displayWidthForSettings(g_frontendSettings);
    int baseHeight = displayHeightForSettings(g_frontendSettings);
    int scale = baseWidth > 0 && baseHeight > 0 ?
        std::min(width / baseWidth, height / baseHeight) : 2;
    if (scale < 2) scale = 2;
    if (scale > 8) scale = 8;
    return scale;
}
...
static bool rebuildFpsOverlayTexture(int displayedFps)
{
    ...
    const int scale = pcFpsOverlayScale();
...
void frontendApplyVideoSettings(const EmulatorSettings& settings)
{
    ...
    printf("frontend: video settings scale=%d fullscreen=%u anti_aliasing=%s effect=%s brightness=%d contrast=%d gamma=%d saturation=%d minimized_behavior=%s screen_orientation=%s screen_fill=%s show_fps=%u\n",
...
static bool drawFrame(uint16_t* pixels, int displayedFps)
{
    ...
    uint16_t* uploadPixels = frontendProcessFramePixels(
        pixels, effectPixels, antiAliasPixels, SCREEN_WIDTH * SCREEN_HEIGHT, g_frontendSettings);
    if (portraitModeEnabled())
    {
    ...
    int outputHeight = 0;
    SDL_GetRendererOutputSize(g_renderer, &outputWidth, &outputHeight);
    const bool portrait = portraitModeEnabled();
    const int contentWidth = portrait ? SCREEN_HEIGHT : SCREEN_WIDTH;
    const int contentHeight = portrait ? SCREEN_WIDTH : SCREEN_HEIGHT;
    SDL_Rect gameDestination = { 0, 0, outputWidth, outputHeight };
    if (g_frontendSettings && g_frontendSettings->screenFill != SCREEN_FILL_STRETCH &&
        outputWidth > 0 && outputHeight > 0)
    {
        if ((int64_t)outputWidth * contentHeight > (int64_t)outputHeight * contentWidth)
        {
            gameDestination.h = outputHeight;
            gameDestination.w = outputHeight * contentWidth / contentHeight;
            gameDestination.x = (outputWidth - gameDestination.w) / 2;
        }
        else
        {
            gameDestination.w = outputWidth;
            gameDestination.h = outputWidth * contentHeight / contentWidth;
            gameDestination.y = (outputHeight - gameDestination.h) / 2;
        }
    }
    if (g_frontendSettings &&
    ...
    if (portrait)
    {
        SDL_Rect dst =
        {
            gameDestination.x + (gameDestination.w - gameDestination.h) / 2,
            gameDestination.y + (gameDestination.h - gameDestination.w) / 2,
            gameDestination.h,
            gameDestination.w
        };
    ...
    {
        renderResult = SDL_RenderCopy(g_renderer, g_frameTexture, NULL, &gameDestination);
    }
    if (renderResult != 0)
    {
...
bool frontendInit(EmulatorSettings* settings, const char* currentAppPath)
{
    ...
    SDL_SetHint(SDL_HINT_IME_SHOW_UI, (!settings || settings->systemImeDisabled) ? "0" : "1");
    ...
    attachAutotestVirtualController();
    openFirstGameController();
...
void frontendRunLoop(const EmulatorOptions& options)
{
    ...
    uint64_t autotestQuitMs = parsePositiveEnv(
        "DINGOO_PIE_AUTOTEST_QUIT_MS", 0, 0, 60 * 60 * 1000);
    bool autotestQuitRequested = false;
    ...
    while (running && !SDL_AtomicGet(&g_quitRequested))
    {
        ...
        if (!autotestQuitRequested && autotestQuitMs &&
            loopElapsed >= autotestQuitMs)
        {
            autotestQuitRequested = true;
            printf("frontend: autotest quit requested elapsed_ms=%llu\n",
        ...
        runAutotestControllerActions(loopElapsed);
        ...
        while (SDL_PollEvent(&ev))
        {
...
#endif
                if ((g_controllerMappingPending || frontendControllerCalibrationActive()) && !ev.key.repeat &&
            ...
            case SDL_CONTROLLERBUTTONUP:
                if ((!frontendGamePaused() || g_controllerMappingPending ||
                        frontendControllerCalibrationActive()) &&
