...
static const int kInputMappingRowCount =
    (int)(sizeof(kInputMappingRows) / sizeof(kInputMappingRows[0]));
...
static const int kInputMappingIdCalibrationValue = 41404;
static const int kInputMappingIdStartCalibration = 41405;
static const int kInputMappingIdResetCalibration = 41406;
...
{
    int index = id - base;
    if (index < 0 || index >= kInputMappingRowCount)
    {
        return -1;
    }
    return index;
...
void inputMappingUiRefresh(void)
{
    ...
    for (int i = 0; i < kInputMappingRowCount; ++i)
    {
    ...
    bool calibrationActive = g_inputMappingCallbacks.controllerCalibrationActive &&
        g_inputMappingCallbacks.controllerCalibrationActive();
    SetWindowTextW(inputMappingItem(kInputMappingIdCalibrationValue), calibrationActive ?
        (inputMappingChinese() ? L"\u6821\u51c6\u4e2d\u2026" : L"Calibrating\u2026") :
        (g_inputMappingSettings && !g_inputMappingSettings->controllerCalibration.empty() ?
            (inputMappingChinese() ? L"\u5df2\u6821\u51c6" : L"Calibrated") :
            (inputMappingChinese() ? L"\u9ed8\u8ba4" : L"Default")));
    EnableWindow(inputMappingItem(kInputMappingIdStartCalibration), !calibrationActive);
    for (int i = 0; i < kInputMappingRowCount; ++i)
    {
        EnableWindow(inputMappingItem(kInputMappingIdKeyboardBase + i), !calibrationActive);
        EnableWindow(inputMappingItem(kInputMappingIdControllerBase + i), !calibrationActive);
    }
    EnableWindow(inputMappingItem(kInputMappingIdResetKeyboard), !calibrationActive);
    EnableWindow(inputMappingItem(kInputMappingIdResetController), !calibrationActive);
    EnableWindow(inputMappingItem(kInputMappingIdResetCalibration), !calibrationActive);
}
...
static LRESULT CALLBACK inputMappingWindowProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
    {
        ...
        if (keyboardIndex >= 0)
        {
            if (g_inputMappingCallbacks.cancelControllerMapping)
            {
                g_inputMappingCallbacks.cancelControllerMapping();
            }
            if (g_inputMappingCallbacks.cancelControllerCalibration)
            {
                g_inputMappingCallbacks.cancelControllerCalibration();
            }
        ...
        if (id == kInputMappingIdStartCalibration)
        {
            g_keyboardMappingPending = false;
            g_keyboardMappingTarget = 0;
            if (g_inputMappingCallbacks.cancelControllerMapping)
            ...
            if (g_inputMappingCallbacks.beginControllerCalibration)
            {
                g_inputMappingCallbacks.beginControllerCalibration();
            }
            inputMappingUiRefresh();
            return 0;
        }
        if (id == kInputMappingIdResetCalibration)
        {
            if (g_inputMappingCallbacks.resetControllerCalibration)
            {
                g_inputMappingCallbacks.resetControllerCalibration();
            }
            inputMappingUiRefresh();
            return 0;
        }
    ...
    case WM_CTLCOLORSTATIC:
    {
        ...
        bool valueField =
            (id >= kInputMappingIdKeyboardTextBase &&
                id < kInputMappingIdKeyboardTextBase + kInputMappingRowCount) ||
            (id >= kInputMappingIdControllerTextBase &&
                id < kInputMappingIdControllerTextBase + kInputMappingRowCount);
    ...
    const InputMappingUiCallbacks& callbacks)
{
    ...
    createInputMappingChild(g_inputMappingWindow, L"STATIC", zh ? L"\u624b\u67c4" : L"Controller",
    ...
    for (int i = 0; i < kInputMappingRowCount; ++i)
    {
        ...
        createInputMappingChild(g_inputMappingWindow, L"BUTTON", zh ? L"\u8bbe\u7f6e\u624b\u67c4" : L"Set Controller",
            0, 0, controllerButtonX, y - 1, controllerButtonWidth, 26, kInputMappingIdControllerBase + i);
    }

    int bottomY = 48 + kInputMappingRowCount * 34;
    createInputMappingChild(g_inputMappingWindow, L"STATIC",
        zh ? L"\u624b\u67c4\u6447\u6746\u6821\u51c6" : L"Joystick Calibration",
        0, 0, controlX, bottomY + 2, 120, 24, -1);
    createInputMappingChild(g_inputMappingWindow, L"EDIT", L"",
        WS_EX_CLIENTEDGE, ES_READONLY | ES_AUTOHSCROLL,
        controllerX, bottomY, controllerWidth, 24, kInputMappingIdCalibrationValue);
    createInputMappingChild(g_inputMappingWindow, L"BUTTON",
        zh ? L"\u5f00\u59cb\u6821\u51c6" : L"Start Calibration",
        0, 0, controllerButtonX, bottomY - 1, controllerButtonWidth, 26,
        kInputMappingIdStartCalibration);
    bottomY += 36;
    ...
    createInputMappingChild(g_inputMappingWindow, L"BUTTON",
        ...
        0, 0, 310, bottomY + 30, 146, 28, kInputMappingIdResetCalibration);
    ...
    inputMappingUiSetStatus(zh ?
        L"\u53ef\u8bbe\u7f6e\u952e\u76d8\u3001\u624b\u67c4\u6620\u5c04\uff0c\u6216\u5bf9\u624b\u67c4\u6447\u6746\u8fdb\u884c\u6821\u51c6\u3002" :
        L"Configure keyboard/controller mappings or calibrate controller sticks.");
    ShowWindow(g_inputMappingWindow, SW_SHOWNOACTIVATE);
