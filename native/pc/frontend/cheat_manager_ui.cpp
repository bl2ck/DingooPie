...
static std::wstring cheatManagerDisplayName(const CheatRuntimeEntryView& entry)
{
    ...
    return wide.empty() ? std::wstring(cheatManagerChinese() ? L"\u672a\u547d\u540d" : L"Unnamed") : wide;
}
...
static void saveCheatManagerSelection(const CheatRuntimeStatus& status)
{
    ...
    bool changed = emulatorSetCheatFeatureKeysForGame(g_cheatManagerSettings,
...
static void refreshCheatManagerWindow(void)
{
    ...
    if (!status.loaded)
    {
        swprintf(statusText, sizeof(statusText) / sizeof(statusText[0]),
            cheatManagerChinese() ? L"\u672a\u627e\u5230\u5339\u914d\u7684 .cht \u6587\u4ef6" :
            L"No matching .cht file");
    }
    else if (status.shaMismatch)
    {
        swprintf(statusText, sizeof(statusText) / sizeof(statusText[0]),
            cheatManagerChinese() ? L"\u4e0d\u9002\u7528\u4e8e\u5f53\u524d\u6e38\u620f" :
            L"Not for current game");
    }
    else
    {
...
static void createCheatManagerContents(void)
{
    ...
    x += kCheatManagerBottomButtonWidth + kCheatManagerBottomButtonGap;
        createCheatManagerButton(cheatManagerChinese() ? L"\u5e94\u7528" : L"Apply",
...
static LRESULT CALLBACK cheatManagerWindowProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
    {
        ...
        {
            CheatRuntimeStatus status = cheatRuntimeGetStatus();
            if (!g_cheatManagerAppPath.empty())
            {
                cheatRuntimeLoadForGame(status.currentGameSha256.c_str(),
                    g_cheatManagerAppPath.c_str(),
                    g_cheatManagerSettings ? emulatorCheatFeatureKeysForGame(
                        *g_cheatManagerSettings, g_cheatManagerAppPath) :
